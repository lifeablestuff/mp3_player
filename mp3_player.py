...
def main_playing(files,pos):
	global pid
	if pid == 0:
		for x in range(len(contents)):
			print("here is the location: "+str(location))
			pid = sp.Popen([vlc_player,'--intf', 'dummy', 'file:///'+ contents[int(pos)]])
			print(pos, contents)
			if int(pos)+1 == len(contents):
				pos = 0
			else:
				pos += 1
			current = MP3(contents[int(pos)])
...
def brow_cb(wid):
    global location
    global pos
    pos=brow.value() # position of the line selected
    print("pos:"+str(pos))
    name=brow.text(pos) # starts at 1
    out.value(name)
    
    location = contents[int(pos)-1]
    print(location)

def play_cb(wid):
	global pos
	global pid #status of playing
	#    global location #path
	print(pid)
	if pid ==0:
		main_playing(contents,pos-1)
...
def skip_cb(wid,which):
	...
	if which == 'next':
		if pos == len(contents):
			...
			play_a_song(contents[int(pos)-1])
	
	else:
		if pos == 0:
			play_a_song(contents[-1])
		else:
			play_a_song(contents[int(pos)-2])
...
def play_a_song(mp3_file):
    global pid
    print(pid)
    if pid ==0: # if is playing already
